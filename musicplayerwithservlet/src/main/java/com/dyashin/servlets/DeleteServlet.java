...
public class DeleteServlet extends HttpServlet {
	...
	private void processDeleteRequest(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			if (rowsAffected > 0) {
				...
				out.println("<p class=\"success-message\">Song with Music ID <strong>" + musicIdToDelete
				...
				out.println("<p class=\"info-message\">No song found with Music ID <strong>" + musicIdToDelete
						+ "</strong>, or no changes were made.</p>");
			}

		} catch (NumberFormatException e) {
			// Handle invalid ID format gracefully
			System.err.println("Invalid Music ID format received for deletion: " + req.getParameter("id") + " - "
					+ e.getMessage());
